...
public class Cell {
    // Content of this cell (empty, cross, nought (O) is called from the Player class.
    Player content;
    ...
    public void paint(Graphics g) {
     ...
     // Calculate the starting position (top-left corner) of the cell using the cell size variable from GameMain Class.
        int x1 = col * GameMain.CELL_SIZE + GameMain.CELL_PADDING;        
        int y1 = row * GameMain.CELL_SIZE + GameMain.CELL_PADDING;
     // Check the content of the cell with an if statement  
        if (content == Player.Cross) {
        	// check if the content is Cross using Player class, set the color to red
         ...
         // Draw two diagonal lines that will create the Cross symbol
        	...
        	// If the content is Nought (O), set the color to blue
            graphic2D.setColor(Color.BLUE);
         // Draw a blue oval to form the Nought symbol (O). This was allot less work to accomplish.
    ...
    public void clear() {
        content = Player.Empty; // Set the value of content to Empty using the enum from Player class.
    }
}
